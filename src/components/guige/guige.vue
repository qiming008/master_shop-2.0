...
export default{
  ...
  mounted(){
    var that = this
    axios.get(`/stewardshop/selecthostvice?id=${this.spid}`)
    .then(function (res) {
      if(res.data.code==200){
        that.guige = res.data.data
        that.sel_fun()
        if(that.guige.length<1){
          axios.get(`/stewardshop/userselectspecification?type=0&stewardid=${that.spid}`)
          .then(function (res) {
            if(res.data.code==200){
              that.price = res.data.data.price
              that.kc = res.data.data.inventory
              that.cont_id = res.data.data.combineid
              that.kcchange = false
            }else{
              Toast(res.data.msg);
            }
  ...
  methods:{
    ...
    sel_fun(){
      if(this.guige.length==1){
        ...
        var that = this
        axios.get(`/stewardshop/userselectspecification?type=1&viceidone=${this.selid0}&stewardid=${this.spid}`)
        .then(function (res) {
          if(res.data.code==200){
            that.price = res.data.data.price
            that.kc = res.data.data.inventory
        ...
        if(this.selid0==-1||this.selid1==-1){
          ...
          var that = this
          axios.get(`/stewardshop/userselectspecification?type=2&viceidone=${this.selid0}&vicetwo=${this.selid1}&stewardid=${this.spid}`)
          .then(function (res) {
            if(res.data.code==200){
              that.price = res.data.data.price
              that.kc = res.data.data.inventory
    ...
    qd(){
      if(this.kg==1){
        ...
        if(this.rk==1){
          ...
          var that = this
          axios.get(`/shopcart/set_shop_cart/${appkey}?storeid=${this.shop.did}&userid=${this.buluo.uid}&count=${this.value}&shopid=${this.spid}&briefid=${this.cont_id}`)
          .then(function (res) {
            if(res.data.code==200){
          ...
          this.list = [{
            ...
            price:this.price,
            count:this.value,
            img:JSON.parse(this.$route.query.goods).image,
            name:JSON.parse(this.$route.query.goods).name,
