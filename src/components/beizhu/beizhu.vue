...
<transition name="slide">
  <div class="cont">
    ...
    <div class="pad_cont">
      ...
      <div class="in_txt">
        <p class="p3">备注：</p>
        <textarea placeholder="请输入备注信息,不超过50字" rows="6" v-model="beizhu" maxlength="50"></textarea>
...
export default {
  data() {
    return {
      ...
      beizhu:this.$route.query.sp.note,
      active: false,
  ...
  methods: {
    bz() {
      if(this.active){
        this.$router.replace({path:'/ordsure',query:{list:JSON.stringify(this.list),routerlist:JSON.stringify(this.routerlist),buluo:JSON.stringify(this.buluo),shop:JSON.stringify(this.shop),beizhu:this.beizhu,index:this.index}})
      }
    }
  },
  mounted(){
    if(this.beizhu){
      this.biezhu = this.beizhu.replace(/^\s+|\s+$/g,"")
      this.active = true
    }else{
      this.active = false
    }
  ...
  watch: {
    beizhu() {
      this.beizhu.length > 0 ? (this.active = true) : (this.active = false);
    }
  }
};
</script>
