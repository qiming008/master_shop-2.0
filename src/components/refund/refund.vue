...
export default {
  ...
  mounted(){
    if(this.$route.query.rk == 1 ){
      this.reason = this.$route.query.liyou
      this.refund_id = this.$route.query.id
    }
  },
  methods: {
    sure() { 
      ...
      let url;
      if(this.rk ==0){
        url = `/api/order/refund/up?order_line_id=${this.info.id}&liyou=${reason}`
      }else{
        url = `/api/order/refund/update/liyou?id=${this.refund_id}&liyou=${reason}`
      }
      if (reason === '') {
        Toast('请输入退款原因')
      } else {
