  ...
  <div class="warp" ref="warp">
    ...
    <div class="wk">
      ...
      <p class="spprice" v-else>
        <span>¥</span>
        <span v-html="goods.minprice"></span>
        <span v-if="goods.maxprice > goods.minprice">- ¥</span>
        <span v-html="goods.maxprice" v-if="goods.maxprice > goods.minprice"></span>
      </p>
      <div class="sptitle_box van-hairline--bottom">
        <p class="sptitle" v-html="goods.name"></p>
      </div>
      ...
      <div class="spdetail">
        ...
        <div class="detInner" v-html="goods.content" ref="inner"></div>
      </div>
...
export default {
  ...
  mounted(){
    ...
    var that = this
    axios.get(`/stewardshop/userselectshopdetile?id=${this.spid}`)
    .then(function (res) {
  ...
  methods: {
    ...
    go_sel(n){
      this.$refs.warp.style.overflowY = 'hidden'
      this.$router.push({path:"/guige",query:{spid:this.goods.id,price:this.goods.minprice,name:this.goods.name,sel0:this.sel0,sel1:this.sel1,selid0:this.selid0,selid1:this.selid1,selval0:this.selval0,selval1:this.selval1,value:this.number,sum:this.goods.sum,rk:n,goods:JSON.stringify(this.goods)}})
    },
    submint(){
      if(!this.btn_kg){
        return 
    ...
    share(){
      ...
      document.location = `js/share/{"title":"${this.goods.brief.name}","desc":"${this.buluo.blname}管家小店","link":"${url}"}`
    }
  },
  components: {
    top
