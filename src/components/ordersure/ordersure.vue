        ...
        <div class="shop_box">
          ...
          <ul>
            <li v-for="(item, index) in goods" :key="item.id">
              ...
              <p @click="add_beizhu(item,index)" class="tit">备注
                <span class="info" v-if="item.note" v-html="item.note"></span>
                <span class="info" v-else v-html="'请输入备注信息'"></span>
              </p>
            </li>
          </ul>
          <div class="total_box">
...
export default {
  ...
  methods: {
    ...
    add_beizhu(item,index) {
      this.$router.push({path: '/beizhu',query:{sp:item,list:this.$route.query.list,routerlist:this.$route.query.routerlist,buluo:this.$route.query.buluo,shop:this.$route.query.shop,index:index}})
    },
    zhifu() {
      if(!this.zf_kg){
        return
      ...
      let parm = qs.stringify({uid:`${this.buluo.uid}`,address_id:`${this.addr.id}`,buluoid:`${this.buluo.blid}`,shop_id:`${this.shop.did}`,note:'',goods:`${JSON.stringify(this.routerlist)}`,provinceid:`${this.shop.shengid}`,cityid:`${this.shop.shiid}`,areaid:`${this.shop.xianid}`,buluo_name:`${this.buluo.blname}`})
      axios.post(`/api/order/up`,parm)
      .then(function (res) {
        that.zf_kg = true
        if(res.data.code==200){
          that.shadow_show = true
          that.pay_show = true
          that.price = Number((res.data.data.money).toFixed(2)),
    ...
    pay(){
      ...
      axios.get(`/api/payorder/wechat/pay?pay_id=${this.id}`)
      .then( (resp) => {
        that.zf_kg = true
        if(resp.data.code==200){
          // console.log(res.data.data +`&redirect_url=${url}`)
          window.location.replace(resp.data.data +`&redirect_url=${url}`)
        }else{
          Toast(resp.data.msg);
        }
      })
  ...
  watch:{
    $route(to,from){
      if(from.path.indexOf('beizhu')!=-1){
        this.beizhu =  this.$route.query.beizhu
        this.index = this.$route.query.index
        this.goods[this.index].note = this.beizhu
        this.routerlist[this.index].note=this.beizhu
        return 
      }
