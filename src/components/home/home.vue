<template>
  <div class="cont">
    ...
    <div class="index_cont">
      <div class="master_card" v-if="gj.status==1||gj.status==2" @click="open_fun">
      ...
      <div class="gjjx"  v-if="gj.status==1">
        <i class="bef"></i>
        <p>管家精选</p>
        <i class="aft"></i>
      </div>
      <ul class="goods_list" v-if="gj.status==1">
        <van-list
          v-model="loading"
          :finished="finished"
          @load="onLoad"
        >
          <li v-for="item in list" @click="go_xq(item)" :key="item.id">
            ...
            <p class="goods_value">￥{{item.minprice}}<span v-if="item.maxprice>item.minprice">起</span></p>
          </li>
        </van-list>
      </ul>
      <!-- <div class="guanting" v-if="gj.status==2"></div> -->
      <div class="weikaitong" v-if="gj.status==4"></div>
      <div class="zanting" v-if="gj.status==2"></div>
      <div class="zhuangxiu" v-if="gj.status==5||gj.status==3"></div>
    </div>
  ...
  methods:{
    get_data(){
      ...
      if(typeof(this.$route.query.uid) != "undefined"){
        ...
        url = `/store/get_app_store/${appkey}?buluo_id=${this.$route.query.blid}`
        this.$store.commit('set_buluo', this.buluo1)
        this.$store.commit('set_wx', this.$route.query.wx)
      }else{
        url = `/store/get_app_store/${appkey}?buluo_id=${this.buluo.blid}`
      }
      var that = this
      axios.get(url)
      .then(function (res) {
    ...
    get_List(){
      var that = this
      axios.get(`/stewardshop/userselectshop?storeid=${this.shop_id}&page=${this.page}&pagesize=10`)
      .then(function (res) {
        if(res.data.code==200){
