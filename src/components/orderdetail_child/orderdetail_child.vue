<template>
  <transition name="fade">
    <div class="order_detail">
      ...
      <!-- <van-pull-refresh v-model="isLoading" @refresh="onRefresh" v-if="detail && detail.order"> -->
        <div class="detail_box" v-if="detail.order" :class="{'detail_padd':(time > 0 && detail.order.status === 0) || detail.order.status < 4}">
          ...
          <div>
            ...
            <div class="order_state" v-if="refundInfo.orderRefund.status === 11 && refundInfo.orderRefund.suo === 0">
              <h1 class="tit">退款申请中</h1>
              <p class="desc">请耐心等待退款处理</p>
            </div>
            <div class="order_state" v-if="refundInfo.orderRefund.status === 1 && refundInfo.orderRefund.suo === 0 &&refundInfo.orderRefund.type === 16 && refundInfo.orderRefund.express === 0">
              <h1 class="tit">请退货</h1>
              <p class="desc">对方同意退款给您，请在5天内将需要退款的货物寄出。退货地址：<span v-html="regoods_address"></span></p>
            </div>
            <div class="order_state" v-else-if="refundInfo.orderRefund.status === 2 && refundInfo.orderRefund.suo === 0">
              <h1 class="tit">退款申请被拒绝</h1>
           ...
           <div class="order_state" v-else-if="refundInfo.orderRefund.status === 2 && refundInfo.orderRefund.suo === 2">
            ...
            <div class="order_state" v-else-if="refundInfo.orderRefund.status === 10">
              <h1 class="tit">退款关闭</h1>
            </div>
            <!--add-->
            <div class="order_state" v-else-if="refundInfo.orderRefund.status === 1 && refundInfo.orderRefund.suo === 0 && refundInfo.orderRefund.type === 16 && refundInfo.orderRefund.express === 1">
              <h1 class="tit">退货已寄回，等待对方收货</h1>
            </div>
            <div class="order_state" v-else-if="refundInfo.orderRefund.status === 21 && refundInfo.orderRefund.suo === 0 ">
              <h1 class="tit">收货后拒绝退款</h1>
              <p class="desc">对方收货后拒绝退款给您</p>
            </div>
          </div>
              ...
              <li v-for="item in detail.orderLineList" :key="item.orderLine.id">
                <div class="item">
                  ...
                  <div class="shopinfo">
                    ...
                    <div class="guige_box">
                      <span class="num" v-if="item.orderRefund.status <= 2">x{{item.orderLine.count}}</span>
      ...
      <!-- </van-pull-refresh> -->
      <div style="background-color:#fff;z-index:100" v-if="detail && detail.order && refundInfo.orderRefund.express != 1 && refundInfo.orderRefund.status != 3 && refundInfo.orderRefund.suo != 1 && refundInfo.orderRefund.status != 10">
        <div class="btn_box" v-if="refundInfo.orderRefund.suo == 3 || (refundInfo.orderRefund.status === 1 && refundInfo.orderRefund.suo === 0 &&refundInfo.orderRefund.type === 16 && refundInfo.orderRefund.express === 0)">
          <div class="org_btn" @click="regoods(detail.orderLineList)">退货</div>
          <div class="org_btn" @click="cancel_refund(refundInfo.orderRefund.id)">取消退款</div>
        </div>
        <div class="btn_box" v-else-if="refundInfo.orderRefund.status == 2 && refundInfo.orderRefund.suo === 0">
          <div class="org_btn" @click="kefu(refundInfo.orderRefund.id)">客服介入</div>
          <div class="org_btn" @click="refund_one(refundInfo.orderLine)">修改退款申请</div>
          <div class="org_btn" @click="cancel_refund(refundInfo.orderRefund.id)">取消退款</div>
        </div>
        <div class="btn_box" v-else-if="refundInfo.orderRefund.status == 21">
          <div class="org_btn" @click="kefu(refundInfo.orderRefund.id)">客服介入</div>
          <div class="org_btn" @click="cancel_refund(refundInfo.orderRefund.id)">取消退款</div>
        </div>
        <div class="btn_box" v-else-if="refundInfo.orderRefund.status == 0 ||refundInfo.orderRefund.status == 11 ">
          <div class="org_btn" @click="cancel_refund(refundInfo.orderRefund.id)">取消退款</div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    get_detail() {
      let that = this
      var url = `/api/order/refund/detail?id=${this.id}`
      axios.get(url)
      .then((res) => {
    ...
    get_regoods_address(){
      axios.get(`/maddress/get_default_address?appkey=${appkey}&mid=${this.detail.order.chang_id}`)
      .then((res) => {
        if(res.data.code===200){
          this.regoods_address = res.data.data.xx_address
        }else{
          Toast(res.data.msg)
        }
      })
    ...
    refund_one(item) { //申请退款：待派送单个商品申请退款/用户取消退款申请后再次
      this.$router.push({path: '/refund', query: {info: item, rk:1,liyou:this.refundInfo.orderRefund.detail,id:this.refundInfo.orderRefund.id}})
    },
    regoods(item) { //申请退货
