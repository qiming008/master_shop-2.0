      ...
      <van-pull-refresh v-model="isLoading" @refresh="onRefresh">  
        ...
        <ul v-if="cont.length > 0">
          <van-checkbox-group v-model="result" @change="checkone">
            <li v-for="(item,index) in cont" :key="item.id">
...
export default {
  ...
  methods: {
    ...
    shuliang:function(id,count){
      var that = this
      axios.get(`/shopcart/update_shop_cart_num/${appkey}?id=${id}&count=${count}`)
      .then(function (res) {
        if(res.data.code==200){
    ...
    ondel:function(){
      ...
      axios.get(`/shopcart/del_shop_cart/${appkey}?id=${this.res_arr_del.join(',')}`)
      .then(function(res){
        if(res.data.code==200){
          Toast.success('操作成功')
          that.jiazai()
