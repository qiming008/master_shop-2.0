<template>
  <transition name="fade">
    <div class="order_detail">
                ...
                <div class="item">
                  ...
                  <div class="shopinfo">
                    ...
                    <div class="price_box">
                      ...
                      <span class="tuikuan_btn" @click="check_one(item.orderRefund)" v-else-if=" detail.order.status === 3 && detail.count != 0 ">查看退款</span>
                    </div>
                  </div>
                </div>
                <p class="tit1">备注<span class="info" v-html="item.orderLine.note?item.orderLine.note:'暂无备注'"></span></p>
              </li>
            </ul>
            <div class="total_box">
      ...
      <!-- </van-pull-refresh> -->
      <div style="background-color:#fff;z-index:100" v-if="detail && detail.order ">
...
export default {
  ...
  methods: {
    get_detail() {
      axios.get(`/api/order/detail?order_id=${this.id}`)
      .then((res) => {
    ...
    pay() { // 待付款立即支付
      ...
      axios.get(`/api/payorder/up?id=${this.id}`)
      .then( (res) => {
        if(res.data.code==200){
          axios.get(`/api/payorder/wechat/pay?pay_id=${res.data.data.id}`)
          .then( (resp) => {
            that.zf_kg = true
    ...
    refund_one(item) { //申请退款：待派送单个商品申请退款/用户取消退款申请后再次
      this.$router.push({path: '/refund', query: {info: item,rk:0}})
    },
    check_one(item) { //查看退款
      this.$router.push({path: '/tk', query: {id:item.id,buluo:this.$route.query.buluo,shop:this.$route.query.shop}})
    },
    regoods(item) { //申请退货
